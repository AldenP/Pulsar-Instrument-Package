...
class SerialMonitor:
    def __init__(self, port="COM10", baud_rate=921600):
        self.serial_port = port
        self.baud_rate = baud_rate
        self.ser = serial.Serial(self.serial_port, self.baud_rate, timeout=5)
        self.ser.set_buffer_size(128*1024)  # increase buffer size
    ...
    def _serial_thread(self):
        """Thread for reading and writing serial data."""
        while self.running:
            if self.ser.in_waiting:
                line = self.ser.readline()
                try:
                    data = str(line, encoding="utf-8").strip()
...
if __name__ == "__main__":
    ...
    def plot_pcnt_data(time_data: array, pcnt_data: dict[int, array], sample_frequency: int):
        ...
        for ch in range(4):
            ...
            pulse_diffs[-2:-1] = 0  # zero the last data points to prevent outlier/bad data
    ...
    while True:
        ...
        if dataIn == b'':
            ...
            p = multiprocessing.Process(target=plot_pcnt_data, args=(pcnt_data[0], pcnt_data[1], sample_freq,))
            p.start()
            processes.append(p)
            p.join()
